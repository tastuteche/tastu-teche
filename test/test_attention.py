...
from tastu_teche.custom_recurrents import AttentionDecoder
...
def get_pair(n_in, n_out, cardinality, verbose=False):
    ...
    sequence_in = sequence_in[:real_length] + \
        [0 for _ in range(n_in - real_length)]
    sequence_out = [sequence_in[real_length - 1]] + \
        [sequence_in[0]] + [0 for _ in range(n_in - 2)]
    if verbose:
...
def attention_model(n_timesteps_in, n_features):
    model = Sequential()
    model.add(LSTM(150, input_shape=(n_timesteps_in,
...
#         model, n_timesteps_in, n_timesteps_out, n_features)
